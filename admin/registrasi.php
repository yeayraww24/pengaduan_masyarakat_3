...
<?php 
	if(isset($_POST['updateStatus'])){
		...
		$status = $_POST['status'];
		if($status == '1'){
			$q = mysqli_query($koneksi, "UPDATE masyarakat SET verif = '1' WHERE nik = '$nik'");
		}else{
			$q = mysqli_query($koneksi, "UPDATE masyarakat SET verif = '0' WHERE nik = '$nik'");
		}
	}
?>
</head>
<div class="row">
	...
	<?php 
		...
		<tr>
			...
			<td><?php if ($r['verif'] == '0') {
                                                        echo '<form action="" method="post"><input name="nik" type="hidden" value="' . $r['nik'] . '"><input name="status" type="hidden" value="1"><button name="updateStatus" style="backgorund-color: green;" type="submit" class="btn btn-danger">Verifikasi</button></form>';
                                                    } else {
                                                        echo '<form action="" method="post"><input name="nik" type="hidden" value="' . $r['nik'] . '"><input name="status" type="hidden" value="0"><button name="updateStatus" style="backgorund-color: red;" type="submit" class="btn btn-success">Blokir</button></form>';
                                                    } ?>
                                            </td>
...
<!-- ------------------------------------------------------------------------------------------------------------------------------------ -->
		...
		<?php 
				if(isset($_POST['simpan'])){
					...
					$query = mysqli_query($koneksi, "INSERT INTO masyarakat VALUES ('$nik', '$nama', '$username', '$password', '$telp', 'masyarakat', '0')");
					if($query){
